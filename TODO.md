...
- [x] **Add Firebase App Check (reCAPTCHA v3)** — register site, init on client, verify token in handler
  - **Implemented (code):** client init in `src/api/appCheck.ts` (dynamic-imported
    ...
    `getAppCheck().verifyToken`, gated by a **LaunchDarkly flag**
    (`app-check-enforcement`: off / monitor / enforce) so behaviour can change
    without a redeploy and fails closed to `enforce`. See `LAUNCHDARKLY.md`.
  ...
  - **Remaining manual steps:**
    1. ✅ Web app + reCAPTCHA v3 registered; Firebase config + site key set as repo
       Variables and in local `.env`; `LAUNCHDARKLY_SDK_KEY` set as a Firebase secret.
    2. Merge + deploy (CI). Flag starts in `monitor` → real traffic sends tokens,
       nothing is rejected; watch the `App Check monitor` logs (`tokenValid`).
    3. Flip the `app-check-enforcement` default rule to `enforce` in production
       (LaunchDarkly dashboard) once monitor logs look healthy — no redeploy needed.
